(function initialiseScanner() {
  ...
  function loadStoredEntries() {
    if (!Core) return [];
    try {
      return Core.loadEntriesPayload(localStorage.getItem(STORAGE_KEY));
    } catch (error) {
  ...
  function saveEntries() {
    try {
      ...
      console.warn('Scanlijst kon niet worden opgeslagen.', error);
      setStatusDetail('De kaart is herkend, maar lokaal opslaan is mislukt. Exporteer de lijst zo snel mogelijk.');
      return false;
    }
  }
  ...
  function buildSetCodeOcrImage(cardCanvas) {
    // Most TCG cards print the set-code directly below the artwork. The second band covers layout variants.
    const upperBand = cropFraction(cardCanvas, 0.43, 0.555, 0.54, 0.125);
    const lowerBand = cropFraction(cardCanvas, 0.43, 0.635, 0.54, 0.115);
    const upper = preprocessTextCanvas(upperBand, { targetWidth: 1400, binary: true });
    const lower = preprocessTextCanvas(lowerBand, { targetWidth: 1400, binary: true });
    const gap = 30;
    const output = createCanvas(Math.max(upper.width, lower.width), upper.height + lower.height + gap);
    const context = output.getContext('2d', { alpha: false });
    context.fillStyle = '#ffffff';
    context.fillRect(0, 0, output.width, output.height);
    context.drawImage(upper, 0, 0);
    context.drawImage(lower, 0, upper.height + gap);
    return output;
  }

  async function readSetCode(cardCanvas) {
    const processed = buildSetCodeOcrImage(cardCanvas);
    const result = await runOcr(processed, {
      tessedit_pageseg_mode: '6',
      tessedit_char_whitelist: 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789- ',
      preserve_interword_spaces: '1'
    }, { start: 62, end: 86 });
    const rawText = String(result && result.data && result.data.text || '').trim().toUpperCase();
    const candidates = Core ? Core.extractSetCodeCandidates(rawText) : [];
    updateDebug({
      'Setcode-OCR': rawText || 'geen tekst',
      'Setcodekandidaten': candidates.join(' | ') || 'geen',
      'Setcode-confidence': Math.round(Number(result && result.data && result.data.confidence) || 0)
    });
    return { rawText, candidates };
  }
  ...
  async function recogniseCard(cardCanvas, runId) {
    ...
    setScanStage('Set-code lezen', 'info', 60, 'Stap 3 van 3: de kaartdruk bepalen met de set-code.');
    const setCodeResult = await readSetCode(cardCanvas);
    assertActiveRun(runId);
